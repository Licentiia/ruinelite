...
@Slf4j
@PluginDescriptor(
        name = PluginDescriptor.Zebe + " NPC Attack Style",
        description = "Draws a custom label above NPCs based on ID (or name, if enabled).",
        tags = {"npc","label","overhead","pvm","pvp", "zebe", "cb"}
)
public class NpcAttackPlugin extends Plugin
{
    @Inject private Client client;
    @Inject private OverlayManager overlayManager;
    @Inject private NpcAttackOverlay overlay;
    @Inject private NpcAttackConfig config;

    @Getter
    private final Map<Integer, String> idToLabel = new HashMap<>();

    @Getter
    private final Map<String, String> nameToLabel = new HashMap<>();

    @Getter
    private final Map<String, Color> labelToColor = new HashMap<>();
    ...
    @Provides
    NpcAttackConfig provideConfig(ConfigManager configManager)
    {
        return configManager.getConfig(NpcAttackConfig.class);
    }
    ...
    {
        if (!e.getGroup().equals(NpcAttackConfig.GROUP)) return;
        reloadAllMaps();
    }
